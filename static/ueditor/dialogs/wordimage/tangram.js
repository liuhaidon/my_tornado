 ...
 * @config1 {Object} vars 创建imageUploader时所需要的参数
 * @config1 {Number} vars.gridWidth 每一个预览图片所占的宽度，应该为flash寛的整除
 * @config1 {Number} vars.gridHeight 每一个预览图片所占的高度，应该为flash高的整除
 * @config1 {Number} vars.picWidth 单张预览图片的宽度
 * @config1 {Number} vars.picHeight 单张预览图片的高度
 * @config1 {String} vars.uploadDataFieldName POST请求中图片数据的key,默认值'picdata'
 * @config1 {String} vars.picDescFieldName POST请求中图片描述的key,默认值'picDesc'
 * @config1 {Number} vars.maxSize 文件的最大体积,单位'MB'
 * @config1 {Number} vars.compressSize 上传前如果图片体积超过该值，会先压缩
 * @config1 {Number} vars.maxNum:32 最大上传多少个文件
 * @config1 {Number} vars.compressLength 能接受的最大边长，超过该值会等比压缩
 * @config1 {String} vars.url 上传的url地址
 * @config1 {Number} vars.mode mode == 0时，是使用滚动条，mode == 1时，拉伸flash, 默认值为0
 ...
 * @config1 {Object} createOptions 创建flash时需要的参数，请参照baidu.swf.create文档
 * @config1 {String} createOptions.width
 * @config1 {String} createOptions.height
 * @config1 {Number} maxNum 最大可选文件数
 * @config1 {Function|String} selectFile
 * @config1 {Function|String} exceedMaxSize
 * @config1 {Function|String} deleteFile
 * @config1 {Function|String} uploadStart
 * @config1 {Function|String} uploadComplete
 * @config1 {Function|String} uploadError
 * @config1 {Function|String} uploadProgress
 */
 ...
 * @config1 {String} [charset] script的字符集
 * @config1 {Integer} [timeOut] 超时时间，超过这个时间将不再响应本请求，并触发onfailure函数
 * @config1 {Function} [onfailure] timeOut设定后才生效，到达超时时间时触发本函数
 * @remark
 ...
 * @config1 {string} [charset] script的字符集
 * @config1 {string} [queryField] 服务器端callback请求字段名，默认为callback
 * @config1 {Integer} [timeOut] 超时时间(单位：ms)，超过这个时间将不再响应本请求，并触发onfailure函数
 * @config1 {Function} [onfailure] timeOut设定后才生效，到达超时时间时触发本函数
 * @remark

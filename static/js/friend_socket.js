    ...
    start: function () {
        ...
        if (window.WebSocket && !MySocket.socket) {
            ...
            MySocket.socket.onopen = function (event) {
                console.log("web开始333。。。")
                var myid = $(".head.flex.flex-pack-justify").data("dt");
                var nick = $(".head.flex.flex-pack-justify").data("nick");
                // var id = $(".header").data("dt");
                var userobj = {"from": myid, "msgid": 2, "content": nick, "to":"oXofH5amJjHGypgyQWSh1kBE5Txk"};
            ...
            MySocket.socket.onmessage = function(event) {
                ...
                if(rs["msgid"] == 4){ /*popup a challege window!*/
                    ...
                    $(".rival2_txbg.flex.flex-v.flex-align-center.flex-pack-center img").attr("src",robert["avatarUrl"]);
